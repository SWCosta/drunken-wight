module CupsHelper
  ...
  def show_match_caster(match)
    debugger unless request.path =~ /gruppen/
  ...
  def edit_match_caster(match)
    ...
    return match_path(match.becomes(GroupMatch), stage_id: params[:stage_id]) if request.path =~ /^\/gruppen/
    return match_path(match.becomes(PlayOffMatch), stage_id: params[:stage_id]) if request.path =~ /^\/\w*finale/
    return edit_final_match_path if request.path =~ /^\/finale/
    match
  end
